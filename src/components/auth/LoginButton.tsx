...
export function LoginButton() {
  ...
  return (
    <button
      ...
      aria-label="카카오톡으로 로그인하여 시작하기"
    >
      {isLoading ? (
        <>
          <div className="h-5 w-5 border-3 border-[#3A1D1D]/20 border-t-[#3A1D1D] rounded-full animate-spin" aria-hidden="true" />
          <span>잠시만 기다려주세요...</span>
        </>
      ) : (
        <>
          <svg className="h-5 w-5" viewBox="0 0 18 18" fill="none" role="img" aria-hidden="true">
            <title>Kakao Icon</title>
            <path
